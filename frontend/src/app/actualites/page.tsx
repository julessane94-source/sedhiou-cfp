...
export default async function ActualitesPage() {
  ...
  if (!actualites.length) return <div className="pt-32 text-center text-gray-600">Aucune actualité pour le moment.</div>
  return (
    <div className="pt-32 pb-20 px-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl md:text-5xl font-bold text-center text-gray-900 mb-12">Actualités</h1>
        <div className="space-y-8">
          {actualites.map((a) => (
            <div key={a._id} className="bg-white rounded-xl shadow-md p-6 transition hover:shadow-lg">
              ...
              <Link href={`/actualites/${a.slug}`} className="text-gray-900 font-semibold hover:underline">Lire la suite →</Link>
            </div>
          ))}
        </div>
      </div>
