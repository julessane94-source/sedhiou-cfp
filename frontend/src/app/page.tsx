  ...
  return (
    <div>
      ...
      <section className="py-12 px-4 bg-[#d6bfbb] text-center">
        ...
        <Link href="/formations" className="inline-block bg-[#772a1d] text-white px-6 py-2 rounded-full">Découvrir nos formations →</Link>
      </section>
      <div className="py-12 px-4 bg-[#d6bfbb]">
        <div className="max-w-4xl mx-auto space-y-6">
          {data.directorMessage?.content && (
            <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md p-5">
              <div className="flex items-center gap-3 mb-3">
                {data.directorMessage.image && <img src={data.directorMessage.image} className="w-12 h-12 rounded-full object-cover border-2 border-[#772a1d]" />}
          ...
          {data.caiMessage?.content && (
            <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md p-5">
              <div className="flex items-center gap-3 mb-3">
                {data.caiMessage.image && <img src={data.caiMessage.image} className="w-12 h-12 rounded-full object-cover border-2 border-[#772a1d]" />}
        ...
        <section className="py-12 px-4 bg-white">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {data.featuredEvents.map((event: any) => (
              <div key={event._id} className="bg-stone-50 rounded-xl shadow-md overflow-hidden">
      ...
      {data.bottomCta && (
        <div className="py-12 px-4 bg-[#772a1d] text-white text-center">
          ...
          <Link href={data.bottomCta.link || '/inscription'} className="inline-block bg-white text-[#772a1d] px-6 py-2 rounded-full">Je m'inscris</Link>
        </div>
      )}
    </div>
  )
