...
export default async function HomePage() {
  ...
  // Support legacy fields (heroTitle, heroSubtitle, heroImage, videoUrl, bottomCta)
  const hero = data.hero ?? {
    title: data.heroTitle || data.hero?.title,
    subtitle: data.heroSubtitle || data.hero?.subtitle,
    backgroundImage: data.heroImage || data.hero?.backgroundImage,
    videoUrl: data.videoUrl || data.hero?.videoUrl,
    ctaText: data.bottomCta?.text || data.hero?.ctaText,
    ctaLink: data.bottomCta?.link || data.hero?.ctaLink,
  }
  return (
    <div>
      {/* Hero */}
      <section className="relative min-h-[85vh] flex items-center justify-center bg-[#d6bfbb] text-stone-800">
        <div className="relative z-10 text-center px-4">
          <h1 className="text-5xl md:text-7xl font-bold mb-4">{hero?.title || 'CFP SEDHIOU'}</h1>
          <p className="text-xl md:text-2xl mb-8">{hero?.subtitle || 'Formez-vous pour un avenir meilleur'}</p>
          <Link href={hero?.ctaLink || '/formations'} className="bg-[#772a1d] text-white px-6 py-3 rounded-full font-semibold hover:bg-gray-100">{hero?.ctaText || 'Découvrir nos formations →'}</Link>
        </div>
      </section>
