      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 animate-slide-up">
            <h2 className="text-2xl font-bold text-white mb-6">CoordonnÃƒÂ©es</h2>
            ...
            <div className="flex space-x-4 mt-6">
              {contact?.facebookUrl && <a href={contact.facebookUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-80 text-2xl">Ã°Å¸â€œËœ</a>}
              {contact?.instagramUrl && <a href={contact.instagramUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-80 text-2xl">Ã°Å¸â€œÂ·</a>}
              {contact?.linkedinUrl && <a href={contact.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-80 text-2xl">Ã°Å¸â€â€”</a>}
              {contact?.twitterUrl && <a href={contact.twitterUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-80 text-2xl">Ã°Å¸ÂÂ¦</a>}
              {contact?.whatsappNumber && <a href={`https://wa.me/${contact.whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="text-white hover:opacity-80 text-2xl">Ã°Å¸â€™Â¬</a>}
            </div>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 animate-slide-up animation-delay-200">
            <form action="/api/contact" method="POST" className="space-y-5">
              ...
              <button type="submit" className="w-full bg-white text-bordeaux-800 font-bold py-2 rounded-lg hover:bg-gray-100 transition transform hover:-translate-y-1">Envoyer le message</button>
            </form>
          </div>
        </div>
      </div>
