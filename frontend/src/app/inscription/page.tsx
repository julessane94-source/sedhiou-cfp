...
const filieresCAP: FiliereCAP[] = ['horticulteur', 'coiffeur', 'couturier modÃ©liste', 'cuisinier', 'dÃ©veloppement local', 'santÃ© hygiÃ¨ne']
...
export default function InscriptionPage() {
  ...
  const handleNiveauChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setNiveau(newNiveau)
    setFiliere('')
    if (newNiveau === 'BT') setMessage('Le niveau BT sera bientÃ´t disponible.')
    else setMessage('')
  }
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (niveau === 'BT') { setStatus('error'); setMessage('Inscriptions BT non encore ouvertes.'); return }
    if (niveau === 'CAP' && formData.niveauEtudes !== '4Ã¨me') { setStatus('error'); setMessage('Pour le CAP, vous devez avoir au moins le niveau 4Ã¨me collÃ¨ge.'); return }
    setStatus('loading')
    try {
      ...
      if (res.ok) {
        setStatus('success'); setMessage('Inscription enregistrÃ©e ! Nous vous contacterons rapidement.')
        setFormData({ nom: '', prenom: '', email: '', telephone: '', niveauEtudes: '' }); setFiliere(''); setAnnee('1')
      } else throw new Error('Erreur serveur')
    } catch (error) {
      setStatus('error'); setMessage('Une erreur est survenue. Veuillez rÃ©essayer plus tard.')
    }
  }
    ...
    <div className="pt-32 pb-20 px-4">
      <div className="container mx-auto max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="card-glass p-6 md:p-8 space-y-6">
          ...
          <div><label className="block font-semibold mb-1">FiliÃ¨re *</label>
          ...
          <div><label className="block font-semibold mb-1">AnnÃ©e (1, 2 ou 3) *</label>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div><label className="block font-semibold mb-1">PrÃ©nom *</label><input type="text" name="prenom" value={formData.prenom} onChange={handleChange} required className="w-full bg-transparent border border-white/30 rounded p-3 text-white placeholder-white/50" /></div>
            ...
            <div><label className="block font-semibold mb-1">TÃ©lÃ©phone *</label><input type="tel" name="telephone" value={formData.telephone} onChange={handleChange} required className="w-full bg-transparent border border-white/30 rounded p-3 text-white placeholder-white/50" /></div>
          </div>
          {niveau === 'CAP' && (
            <div><label className="block font-semibold mb-1">Votre niveau actuel * (CAP exige au moins la 4Ã¨me)</label>
              <select name="niveauEtudes" value={formData.niveauEtudes} onChange={handleChange} required className="w-full bg-transparent border border-white/30 rounded p-3 text-white">
                <option value="">-- SÃ©lectionnez --</option><option value="4Ã¨me">4Ã¨me (ou plus)</option><option value="autre">Moins que 4Ã¨me (non Ã©ligible)</option>
              </select>
              <p className="text-sm text-white/60 mt-1">âš ï¸ Pour le CAP, vous devez avoir validÃ© la 4Ã¨me.</p>
            </div>
          )}
          <button type="submit" disabled={status === 'loading' || niveau === 'BT' || (niveau === 'CAP' && formData.niveauEtudes !== '4Ã¨me')} className="w-full bg-white text-bordeaux-800 font-bold py-3 rounded-lg hover:bg-gray-200 transition disabled:opacity-50">
